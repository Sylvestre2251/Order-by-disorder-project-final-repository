...
class Simulation:
    ...
    def Monte_Carlo(self,Nf,T,measure_capa=False,overrelaxation=False,overrelaxation_dose=0.1):
        ...
        for comp in range(Nf):
            ...
            if  measure_capa==True:
                ...
                sisj1.append(np.sum(np.einsum('...l,...l->...',self.config,self.config)))
                sisj2.append(np.sum(np.power(np.einsum('...l,...l->...',self.config,self.config),2)))
                sisj3.append(np.sum(np.power(np.einsum('...l,...l->...',self.config,self.config),3)))
    ...
    def order_param(self):
        ...
        sisj1=np.sum(np.einsum('...l,...l->',self.config,self.config))
        sisj2=np.sum(np.power(np.einsum('...l,...l->',self.config,self.config),2))
        sisj3=np.sum(np.power(np.einsum('...l,...l->',self.config,self.config),3))
